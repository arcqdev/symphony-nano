defmodule SymphonyElixir.OrchestratorStatusTest do
  ...
  test "token budget kills agent and moves issue to human review when input limit exceeded" do
    ...
    # Use a long-lived process as the fake agent so Process.alive? returns true
    ...
    # Send a token update that exceeds the input budget (100)
    send(
      pid,
      {:codex_worker_update, issue_id,
       %{
    ...
    # Budget exceeded flag should be set and agent killed
    mid_state = :sys.get_state(pid)
    assert MapSet.member?(mid_state.token_budget_exceeded, issue_id)
    refute Process.alive?(agent_pid)

    # Simulate the DOWN message (in production the orchestrator holds the monitor)
    ...
    # Issue should be completed and released
    assert MapSet.member?(state.completed, issue_id)
    refute MapSet.member?(state.claimed, issue_id)

    # Token totals should be cleaned up
  ...
  test "token budget enforces output limit separately from input" do
    ...
    # Input is fine (20 < 10_000) but output exceeds limit (60 > 50)
    send(
      pid,
      {:codex_worker_update, issue_id,
       %{
  ...
  test "token budget accumulates across runs for the same issue" do
    ...
    # Simulate a completed first run that used 120 input tokens
    state_with_prior = %{
      initial_state
    ...
    # This run adds 90 input tokens (120 + 90 = 210 > 200 limit)
    send(
      pid,
      {:codex_worker_update, issue_id,
       %{
