defmodule SymphonyElixir.CoreOrchestratorAndPromptTest do
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "You are working on `symphonyclaw`, the root project shell."
    assert prompt =~ "Follow the repository instructions in this order before making implementation decisions:"
    assert prompt =~ "`AGENTS.md`"
    assert prompt =~ "`plan2.0.md`"
    assert prompt =~ "keep SymphonyClaw narrow on purpose"
    assert prompt =~ "do not add sandbox complexity to the core"
    assert prompt =~ "work only inside the provided issue workspace"
    assert prompt =~ "relevant validation for the touched area is run before handoff"
  end
